...
public class CameraFollow : MonoBehaviour
{
    ...
    public void Start()
    {
        ...
        target = GameObject.FindGameObjectWithTag("Player").GetComponent<SpriteRenderer>();
    ...
    private void SetCameraLimits()
    {
        for (int i = 0; i < limits.Length; i++)
        {
            ...
            if (exitPos.y == 0)
            {
                limits[i].y = (exitPos.y - camLimits);
            }
            else if(exitPos.y < 0)
            {
                limits[i].y = (exitPos.y - camLimits);
            ...
            else if (exitPos.y > 1)
            {
                ...
                {
                    limits[i].y = - camLimits;
                }
                
            }
        }
               
        //Set Minimum Camera Position
        if(limits[0].y > 0)
        {
            minCameraPos = new Vector3(minCameraPos.x, (limits[0].y * -1) + 4, minCameraPos.z);
        }
        else
        {
            minCameraPos = new Vector3(minCameraPos.x, limits[0].y + 10, minCameraPos.z);
            if(minCameraPos.y >= 0)
            {
                minCameraPos.y = limits[0].y; 
            }
        }

        //Set Maximum Camera Position
        maxCameraPos = new Vector3(exitPos.x + 0.6f, limits[0].height, transform.position.z);
    }
